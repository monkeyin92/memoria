...
async def test_self_use_owner_is_confirmed_and_policy_decides_memory(
    ...
    admin = await asyncpg.connect(bootstrap_dsn)
    try:
        # This lineage writes no consent grant at binding: seed the memory
        # grant the self_use memory offer would carry directly.
...
async def test_parent_for_child_confirms_the_child_but_the_guardian_never_reads_memory(
    ...
    # This lineage has no guardian consent writer, so the child's profile
    # stays empty; the guardian's app is never the child talking.
    assert not set(_shape(without)["capabilities"]) & _GATED


@requires_postgres
@pytest.mark.asyncio
async def test_child_for_parent_stays_unconfirmed_without_delegate_consent_discovery(
    postgres_runtime_with_consent: tuple[PostgresSessionRuntimeStore, str],  # noqa: F811
) -> None:
    store, bootstrap_dsn = postgres_runtime_with_consent
    now = datetime.now(UTC)
    ...
    try:
        ...
        # This lineage never attests delegate_for at binding.
    ...
    profile = await _read(_control(store, manifest), manifest)
    # This lineage's consent discovery accepts only guardian_of for another
    # person's consent, so confirming the elder for the adult child would make
    # profile issue fail; the control path leaves the elder unconfirmed.
    assert _shape(profile) == {
        "active_subject_id": None,
        "speaker_state": "unconfirmed",
        "service_mode": "unknown_safe",
