...
def test_memory_projection_rebuild_uses_the_control_api_module_entrypoint() -> None:
    archive_runbook = (ROOT / "docs" / "runbooks" / "release-rollback.md").read_text(encoding="utf-8")
...
def test_agent_component_release_is_commit_bound_thin_and_rollback_safe() -> None:
    ...
    assert "pre-cutover-live.override.yml" in deploy
    assert "validate_component_override" in deploy
    assert "component override is not image-only" in deploy
...
def test_ci_selects_component_gates_and_uses_collision_safe_pytest_imports() -> None:
    ...
    assert "uses: dorny/paths-filter@v4" in workflow
    assert "predicate-quantifier: some-with-excludes" in workflow
...
def test_old_env_with_retired_context_fetch_keys_still_splits_and_routes_them_nowhere() -> None:
    # The agent-side persona capsule fetch, /v1/persona/session-capsule and
    # /v1/archive/session-context were removed on 2026-09-26; live operator env
    ...
    routed_sets = split_env(
        {
            ...
            "MEMORIA_MEMORY_READ_TOKEN": "memory-read-token-material-that-is-long",
        }
    )
    control = routed_sets[0]
    for retired in RETIRED_CONTEXT_FETCH_KEYS:
...
def test_env_templates_drop_the_retired_context_fetch_keys() -> None:
    for template in (ROOT / ".env.example", ROOT / "infra" / "memoria.env.production.example"):
        text = template.read_text(encoding="utf-8")
        for key in RETIRED_CONTEXT_FETCH_KEYS:
