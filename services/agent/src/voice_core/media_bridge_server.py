...
class MediaBridgeSession:
    ...
    _pacing_audio_ms: float = 0.0
    _pacing_max_gap_ms: float = 0.0
    _pacing_queue_high_water: int = 0
    _pacing_logged: bool = False
    ...
    def _record_downlink_pacing(self, frame: PCMFrame) -> None:
        ...
        previous = self._pacing_last_at
        if self._pacing_started_at is None:
            self._pacing_started_at = now
        elif previous is not None:
            self._pacing_max_gap_ms = max(
                self._pacing_max_gap_ms, (now - previous) * 1000.0
            )
        self._pacing_last_at = now
        self._pacing_queue_high_water = max(
            self._pacing_queue_high_water, len(self.downlink)
        )
        if frame.final:
    ...
    def _reset_downlink_pacing(self) -> None:
        self._pacing_started_at = None
        self._pacing_last_at = None
        self._pacing_frames = 0
        self._pacing_audio_ms = 0.0
        self._pacing_max_gap_ms = 0.0
        self._pacing_queue_high_water = 0
        self._pacing_logged = False
    ...
    def accept_downlink(self, frame: PCMFrame) -> bool:
        ...
        self._record_downlink_pacing(frame)
        return True
