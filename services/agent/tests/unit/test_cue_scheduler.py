...
def test_listener_cues_are_bounded_cooled_down_and_history_free() -> None:
    ...
    assert scheduler.observe_partial("我最近想了很多", now_ns=2_700_000_000) is None
    first = scheduler.observe_partial("我最近想了很多事情", now_ns=2_900_000_000)
    assert first is not None
    assert (first.text, first.user_turn_id, first.cue_epoch) == ("嗯", 7, 1)
    assert scheduler.observe_partial("然后呢", now_ns=4_000_000_000) is None

    second = scheduler.observe_partial("我还想继续说", now_ns=7_000_000_000)
    assert second is not None and second.text == "我在听"
    assert scheduler.observe_partial("还有最后一点", now_ns=12_000_000_000) is None
...
def test_new_turn_invalidates_old_cue_epoch() -> None:
    ...
    first = scheduler.observe_partial("第一轮", now_ns=1)
    assert first is not None

    invalidated_epoch = scheduler.cancel_turn()
    scheduler.start_turn(user_turn_id=2, now_ns=2)
    second = scheduler.observe_partial("第二轮", now_ns=3)
