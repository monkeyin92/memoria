...
@pytest.mark.asyncio
async def test_funasr_rotation_survives_provider_initiated_task_finish() -> None:
    """A provider-finish boundary must not wedge rotation or kill recv."""

    srv = MockFunASRServer(scenario="task_reuse_provider_finish")
    srv.start()
    try:
        session = FunASRSession(
            FunASRConfig(
