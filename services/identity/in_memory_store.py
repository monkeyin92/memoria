...
class InMemoryIdentityStore:
    ...
    async def reconcile_account_registration(
        ...
        with self._lock:
            current = self._persons.get(person.person_id)
            ...
            self._persons[person.person_id] = replace(
                current, subject_category="adult", age_band="adult",
                age_evidence_status="verified", updated_at=person.updated_at,
            )
            self._audit.append(audit_event)
            self._enqueue_outbox_locked(outbox_event)
