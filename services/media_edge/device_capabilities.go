...
// ResolveDownlinkRate keeps the device on its native 24 kHz playback clock
// whenever the v2 audio contract offers it. A v2 device that only exposes
// the legacy-compatible 16 kHz option is still supported, while legacy v1
// callers remain fixed at 24 kHz.
