...
WAKE_WORD="${MEMORIA_FIRMWARE_WAKE_WORD:-disabled}"
NO_IDF_INSTALL=0

usage() {
    cat <<'EOF'
Usage: build.sh [--clean] [--language LOCALE] [--wake-word MODEL] [--no-idf-install]

The default bring-up build uses zh-CN and disables the wake word to reduce
hardware variables. The upstream build.py still performs the real merge-bin.
EOF
}
...
while (($# > 0)); do
    case "$1" in
        ...
        --wake-word) shift; (($# > 0)) || die "--wake-word requires a value"; WAKE_WORD="$1" ;;
...
"$python_bin" scripts/build.py "$MEMORIA_BOARD_PATH" \
...
for image in bootloader.bin partition-table.bin xiaozhi.bin; do
