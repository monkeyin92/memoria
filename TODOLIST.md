...
- Agent/工具发布源码已冻结并推送为 `d721455`，2026-09-15 15:58:42 CST 仅切换 Agent/Bridge；后续 `0025` 计量生命周期及捕获/报告修复已完成本地验证，随本次源码与文档冻结，**新固件未刷机、无新捕获、未部署服务端**。9 月 14 日 15:11 的真机回归 **14/14 不是 T1–T14**，也不能继承为新候选的真机验收；`direct_real_device_verified`、`full_duplex_verified` 仍为 false。
...
- **最新状态（2026-09-15，以下历史过程以此为准）**：
  ...
  - [ ] **下一步：新候选真机计量/捕获验收**：受控 app-only 刷写、整槽回读/非 app 区保护、boot 后，用新版工具捕获。实际播放须有 `output_frames>0 / first_output=yes` 及终端 summary；结束时间、逐路退出码/原因与流健康须完整，强杀或缺记录保持 degraded/incomplete。旧 `session-2/capture.json` 已只读复核为 incomplete，receipt 仍 bound，未补造收尾或覆盖历史报告；旧板听感 1/3 不继承给新固件。
  ...
  - **新固件计量候选**：`code=true / wired=true / enabled=false / verified=false_pending_device`；overlay `24531273…b3`、app `7d95c8a1…b65`、ELF `da6ebdd1…69f2`，完整哈希与 `final/` 构建证据见 `HANDOFF.md`。**旧板** overlay `97fc64f2…` / app `dfba3d61…` 已启动且首轮听感通过，但计量未覆盖真实播放；原 `run-20260915-p0-03-firmware-metering/postflash.json` 仅绑定旧板。
  - [ ] 长播验收：新计量候选的设备/捕获验收通过、USB/串口稳定后，再做完整 >45 秒天气/长回复，保留设备 `supply / prestart / boundary / close_dropped / outside`、Bridge pacing、delivery ledger 和操作员完整听感确认。记录计量日志对实际收包/播放时序的影响；`pre-roll` 未实现，不实现 `0026`。桥侧比率、软件队列与 `actual_heard/playback_ended` 均不能单独当作用户完整听完。
...
- **真机第二段（2026-09-14 18:24–18:30，两段会话 `7d9d3a2f` / `e81ff828`）**：修复后的「明天天气」ACK + 正文四条回执全齐、**没有再出现第二个轮次**（该缺陷复现路径未再触发）。但暴露两个新的用户可见问题：
  ...
  - **`0025` 计量实现（本地冻结候选）**：补丁、header、宿主断言和 reset/fence/late-token 防污染逻辑已完成；仅测 decoded playout queue consumer 的 software queue wait，分类为 `supply / prestart / boundary / close_dropped / outside`，并单独记录 exact TX-EOF completion；不宣称 I2S/DMA underrun 或用户实际听感。clean overlay apply、固件宿主测试和 clean build 已通过；新候选尚未刷机，不能继承下两项旧板证据。
