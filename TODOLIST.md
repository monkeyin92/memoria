...
- **真机第二段（2026-09-14 18:24–18:30，两段会话 `7d9d3a2f` / `e81ff828`）**：修复后的「明天天气」ACK + 正文四条回执全齐、**没有再出现第二个轮次**（该缺陷复现路径未再触发）。但暴露两个新的用户可见问题：
  ...
  - **下行节奏测量已上线（2026-09-14 18:45:37 CST，`memoria-agent:20260914-downlink-pacing-v1`）并用一次长天气取到数**：`session=59f4c8ac` gen 3 `frames=1774 audio_ms=35480 wall_ms=35545 max_gap_ms=41 produced_ratio≈0.998`；同会话 gen 1/2 为 `1020/1025ms max_gap=60`、`2660/2639ms max_gap=25`；设备侧同代播放 18:47:54.6→18:48:30.2（35.6s），与 `audio_ms` 一致。**结论：产出几乎正好 1.0x 实时，且相邻帧最大间隔仅 41ms——桥侧从不卡顿，但设备缓冲因此永远接近空**，任何设备/网络抖动都会欠载→断续；账本记「播放完成」只代表最后一帧被消费。修复方向：播前抖动缓冲（pre-roll）或让合成跑在播放前面，但必须与「说完到开口 ≤1.5s」权衡。仍未直接测到设备侧欠载，需要设备侧队列/欠载计数才能把「供给无余量」升级为「确实饿死」。
  - **修复方案已定（2026-09-14，用户选 a）**：长回复断续采用**播前预滚 + 设备侧欠载计量**，不用「合成大幅提前」；设计见 `outputs/design/playback-jitter-buffer-20260914/decision-01-pre-roll-and-measurement.md`。要点：新增 overlay 补丁 `0025`（在 `main/audio/audio_service.cc` 的 `audio_decode_queue_` 上等够 `PRE_ROLL_MS`≈300ms 再启动 I2S），首帧仍按第一片可播帧上报，预期「说完到开口」由 0.696/0.990s 升到约 1.0–1.3s（判据 ≤1.5s，余量约 200ms）；**必须一并重验** `0008/0016/0017` 的播放完成水位与 `0023` 的队列溢出路径，且不得借机放开播放期语音打断。实现（补丁 + `firmware/esp32/tests/` 源码断言 + clean build）、刷机与真机复测仍待做。
  - **固件：先落了「只计量」的 `0025`（已生成、已验证可应用；未编译、未刷机）**。上游树其实在 `firmware/esp32/.cache/upstream`（我之前搜浅了），所以补丁是直接改上游文件后用 `git diff` 机械生成、再用 `git apply --check --reverse` 验证的，格式与既有补丁一致。内容：在 `main/audio/audio_service.cc` 的消费侧（`AudioOutputTask` 等帧分支）统计**真实欠载**——`media playback starved generation=… gap_ms=… starved_count=…`，并在切换代次时汇总 `media playback meter generation=… starved_count=… max_gap_ms=… total_gap_ms=…`；**不改任何控制流**（无死锁/时延风险）。先做这一半的原因：欠载至今没有设备侧证据，而预滚值（300ms）本该由这批数据来定；预滚本身作为 `0026` 紧随其后。剩余未做：宿主机断言、clean build、app-only 刷 `0x20000`、真机复测。
  - **`0025` 已编译验证（2026-09-14）**：补丁先按「叠加 overlay 后的树 vs 再加改动」重建，修掉了把 24 个既有 overlay 改动一起带进来的问题；现在 3 hunk / **0 删除行**（纯新增），`git apply --check --reverse` 通过。`check-overlay.sh` 在干净克隆上报告 `overlay: applied`（hash `9f6d083c…`），`build.sh` 成功产出 `artifacts/memoria-esp-vocat-app.bin`（3,277,856 bytes，sha256 `f8f48a4b…`；上一版 3,277,328 bytes，差值即计量代码），构建后 `overlay check passed: memoria-esp-vocat`，`firmware/esp32/tests` 140 项全绿。**仍未刷机**：板子仍跑 `e1c6998`，下一步是 app-only 写 `0x20000`（刷前全槽回读）+ 真机复测 `starved_count/max_gap_ms`。
  - **`0025` 已刷入真机并逐字节校验（2026-09-14 19:05 CST）**：app-only 只写 `0x20000..0x340fff`；写后全槽回读一致、erase 范围外未变、identity 与非 app 分区及 ota_1/assets MD5 全未变；启动 `activating -> idle`、Activation v3 完好。板载版本 commit `fa54d7d` / app 3,277,856 bytes / SHA `f8f48a4b…`；回滚件与收据在 `outputs/acceptance/run-20260914-p0-03-firmware-metering/`。**下一步：真机问一次长天气**，读 `media playback starved … starved_count/max_gap_ms`，与服务端 `media downlink pacing` 对照，用这批数据定 `0026` 预滚值。
