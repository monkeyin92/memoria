...
**2026-09-22 进展**：已完成只读子集——回顾页展示当前认证主体最近合格对话列表（时间、轮数），点击后查看 owner/assistant 配对内容；服务端新增 subject-scoped `conversation-sessions` 窄读口，复用 `conversation_review` 与 retention 门禁，并固化主体/资格过滤、分页窗口、近似播放记录和 authEpoch 迟到响应保护。**未完成**：首页今日对话次数、每日摘要卡片、一键分享到微信；本轮不把 DEMO-03 整体勾选完成。
