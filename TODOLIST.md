...
- Agent/工具发布源码已冻结并推送为 `d721455`，2026-09-15 15:58:42 CST 仅切换 Agent/Bridge；后续 `0025` 计量生命周期及捕获/报告修复冻结为 `d1ad38f`，**18:51 CST 已 app-only 刷入、回读并启动，18:52 新工具短捕获正常收尾**。本轮未部署服务端；新候选实际播放计量与听感仍待验。9 月 14 日 15:11 的真机回归 **14/14 不是 T1–T14**，也不能继承为新候选的真机验收；`direct_real_device_verified`、`full_duplex_verified` 仍为 false。
...
- **最新状态（2026-09-15，以下历史过程以此为准）**：
  ...
  - [ ] **下一步：新候选真实播放计量/会话捕获验收**：直接使用已启动的新 app，**不重复刷机**。实际播放须有 `output_frames>0 / first_output=yes` 及终端 summary；会话结束时间、逐路退出码/原因与流健康须完整，强杀或缺记录保持 degraded/incomplete。短启动窗口通过不替代会话证据；旧 `session-2/capture.json` 仍保留 incomplete，不补造收尾或覆盖历史报告，旧板听感 1/3 不继承给新固件。
  ...
  - **新固件计量候选**：`code=true / wired=true / enabled=true_boot_verified / verified=false_pending_playback`；overlay `24531273…b3`、app `7d95c8a1…b65`、ELF `da6ebdd1…69f2`，完整哈希、构建与本轮设备收据见 `HANDOFF.md`。**紧邻回滚**为本轮读出的旧 app `dfba3d61…`，旧板首轮听感通过但计量未覆盖真实播放；原 `run-20260915-p0-03-firmware-metering/postflash.json` 仅绑定旧板。
...
- **真机第二段（2026-09-14 18:24–18:30，两段会话 `7d9d3a2f` / `e81ff828`）**：修复后的「明天天气」ACK + 正文四条回执全齐、**没有再出现第二个轮次**（该缺陷复现路径未再触发）。但暴露两个新的用户可见问题：
  ...
  - **`0025` 计量实现（已刷入，实际播放待验）**：补丁、header、宿主断言和 reset/fence/late-token 防污染逻辑已完成；仅测 decoded playout queue consumer 的 software queue wait，分类为 `supply / prestart / boundary / close_dropped / outside`，并单独记录 exact TX-EOF completion；不宣称 I2S/DMA underrun 或用户实际听感。clean overlay apply、固件宿主测试和 clean build 已通过；当前候选已有自己的回读/启动证据，不能继承下两项旧板计量或听感结论。
