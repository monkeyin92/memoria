...
更新于 2026-09-16（第三轮），审查基线 `28a41ee`。本文件只保留未完成事项；完成后把仍有效的运行结论归入 `HANDOFF.md`，并删除任务/子项，不积累完成记录或另建归档。已有编号不复用；生产、设备、回滚与原始证据以 `HANDOFF.md` 为准。

## 下一步与执行边界

1. P0-04 的代码侧证据与数据主体错配已改；真实 PostgreSQL/RLS 契约、repeat-upgrade 门禁和带 DSN 全量测试已通过。下一步是用真实 persistent Session Runtime 把签名 profile 送进 `/response-plan` 与 `/session-policy`，再做授权设备 admission/主体切换验收。
...
本次整理仅改文档，未部署、未刷机、未改生产数据、未连接设备；复核所依据的本机 Docker/PG 验证收据已存在。最新 CI `35092363159` 的 changes、agent-image、agent、python、firmware 通过，miniprogram、media-edge-image、media-edge 跳过；本地/CI 通过不等于真实设备或生产 `enabled/verified`。最新已记录 Agent/Bridge 运行 `d96d4c2`、板卡 `d1ad38f`，不能把 HEAD 的 `code` 记成线上 `enabled/verified`。
...
- 当前有效事实：安静环境阶段 4 已有“茉莉”10/10、5 分钟误唤醒为 0；电视人声与家庭噪声仍没有可比较的漏唤醒/误唤醒 receipt，不能据此宣传家庭场景召回率。
- 工作：在冻结候选和固定固件上测静音、电视人声、多人说话、近/远距离等矩阵，记录真唤醒召回、误唤醒、响应延迟和触发时上下文。只有数据证明必要时才调整 WakeNet threshold 或词形；不开启播放期 KWS，不把目录切词当成 WakeNet 证据。
