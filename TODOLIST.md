...
更新于 2026-09-17（晚），审查基线 `30dea92c0e56983b17a71f8dbbd044a1ecb16798`（本轮新增真实 PG 矩阵测试后；文档同步随 docs commit 进行）。本文件只保留未完成事项及其必要证据边界；完成后把仍有效的运行结论归入 `HANDOFF.md`，并删除任务/子项，不积累完成记录或另建归档。已有编号不复用；生产、设备、回滚与原始证据以 `HANDOFF.md` 中带日期的收据为准，不把旧文档状态当成当前验收。

## 下一步与执行边界

1. P0-04 person-consent lifecycle fixed locally (2026-09-17, a8ce4e1, code+tests+real PG): trusted subject context for the grantor single read/revoke, unbind-proof grantor revoke, idempotent grant replay and SQLite export; P1-01 Mypy blocker cleared. The remaining real-PG matrix for authority unavailable/expired, profile-session mismatch, legal-manager change and concurrent switches is now covered by four new tests in `test_persistent_runtime_policy_chain.py` (2026-09-17, this round). Still open: the under_14/14_17/adult/unknown_safe category matrix at the two seams with a real subject/account-key catalog, old-cache replay beyond switch/epoch races, then wire the P1-03 minimal app_confirm entry; device admission stays the final P0-04 gate.
