...
- **最新状态（2026-09-15，以下历史过程以此为准）**：
  ...
  - [ ] **下一步：查询延迟收口**：epoch 1955 第二问尝试 **21** 个地名才命中南京，查询 **7930ms**、ACK→正文服务端间隔 **5.028s**（设备同钟回 listening→首帧 5.049s），没有慢查询第二提示代，未达 1.5s 标准。已证放大点是 `_location_candidates` 逐字删首字符后由 `_geocode` 串行 await；ASR final 10 字→commit 42 字仅为输入污染线索，来源未定。先以本轮真实样本复现输入拼接/候选链，界定候选数量与总时限并保留正常地名/日期语义，核查既有慢提示为何未覆盖；修改后定向回归和真机时序验收。不放宽防重复门，不以重复 ACK、调长静默或固件预缓冲掩盖延迟。
...
- 剩余条件：本轮功能听感、短播放计量及真实会话收尾已过；**查询延迟未过**（9 月 15 日 epoch 1955 第二问 ACK→正文服务端间隔 5.028s），慢查询第二提示未覆盖。最后一轮功能复测、新 VAD/临近静默、>45s 长播及其它既定专项仍需操作员在场；本地全绿不能替代真机证据。
- 原因：查询地名候选串行放大已由本轮日志与源码证实，输入杂词来源仍未知；当前修复还缺空输入恢复、新 VAD 接管与临近静默的专项真机时序。不要继续把 9 月 13 日的 1.766s 当最新观测，也不把用户无断续反馈当时延通过。
- 工作：先按上方「查询延迟收口」复现并修复，再补「ACK 后空输入 → 有效正文恢复」、同问重复提交、答案完成后再问同题、查询南京时改问北京、慢查询第二提示；修改沿用既有 provider/Router/Ledger/fence，不新增并行状态机。记录提问结束、ACK 起止、正文首帧、终端播放回执，不能只统计服务器首 token。
