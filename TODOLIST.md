...
- 人格分配 UI 已接（`64a742f`，code；远端 CI `35306937489` 的 miniprogram job success：225 passed / 0 fail + 全量 `node --check`；本地同口径）：设备页新增按使用人分配/取消分配，列出 BindingManifest 的主要使用者/家庭成员并显示 override 优先、binding 默认兜底，抽屉从内置伙伴目录（starlight/taoxi/mianmian/axu/xuanmo）选择后 PUT、或 DELETE 恢复默认；写入后回读服务端分配，失败不置成功态。**剩余**：自定义人格（`GET /v1/personas`）不进选择器、成员追加与年龄资料 UI（guardian 页通道已有，需确认是否已覆盖"添加家庭成员"入口）、设备链实听与运行中会话轮换（见上一条）。
