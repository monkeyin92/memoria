...
- 小程序接设备/成员、年龄资料、分配/取消分配/邀请与 app_confirm；先核实际线上 Control 路由/schema，9 月 11 日的两文件 overlay 不代表当前全量后端。客户端 flag、自报姓名或 voice_question 不作身份证据。**本轮已核对当前仓库路由（接线清单）**：分配/取消分配 `GET|PUT|DELETE /v1/devices/{device_id}/persona-assignments[/{person_id}]`（PUT body `{"persona_selection":"<id>:v<n>|cu_…"}`，DELETE 返回 `{"removed":bool,"effective":…}`）；选人 `POST /v1/sessions/resolve-subject`（`client_claimed_person_id` 仅作候选提示）与 `POST /v1/sessions/{session_id}/active-subject`（只接受 `confirmation_method:"app_confirm"`，`voice_question` 仍 422）；成员/关系邀请 `POST /v1/relationships/invites`、`POST /v1/relationships/{id}/{accept,revoke,suspend,resume}`、`GET /v1/relationships`、`GET /v1/persons/me`；设备绑定 `POST /v1/device-bindings`、`GET /v1/devices/{id}/binding`、`POST /v1/devices/{id}/binding/{unbind,supersede}`；自定义人格 `GET|POST /v1/personas`、`DELETE /v1/personas/{persona_id}`；监护人面 `GET /v1/guardian/minors/{minor_user_id}/summary`、`/v1/guardian/{links,notifications}`、`{delete,export}`。线上若与仓库不一致，以线上为准并在此记录差异。
