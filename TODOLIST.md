...
- 小程序接设备/成员、年龄资料、分配/取消分配/邀请与 app_confirm；先核实际线上 Control 路由/schema，9 月 11 日的两文件 overlay 不代表当前全量后端。客户端 flag、自报姓名或 voice_question 不作身份证据。**本轮已核对当前仓库路由（接线清单）**：分配/取消分配 `GET|PUT|DELETE /v1/devices/{device_id}/persona-assignments[/{person_id}]`（PUT body `{"persona_selection":"<id>:v<n>|cu_…"}`，GET 成员可读、写仅 owner/admin，DELETE 返回 `{"removed":bool,"effective":…}`）；选人 `POST /v1/sessions/resolve-subject`（`client_claimed_person_id` 仅作候选提示）与 `POST /v1/sessions/{session_id}/active-subject`（只接受 `confirmation_method:"app_confirm"`，`voice_question` 仍 422）；年龄申报 `PATCH /v1/persons/{person_id}/age-evidence`，本人资料 `GET /v1/persons/me`；**邀请走 `/v1/guardian/links`（绑定码）**，`POST /v1/relationships/invites` 需要 `established_evidence_id`，小程序没有该凭证来源，不作为产品内邀请通道；设备绑定 `POST /v1/device-bindings`、`GET /v1/devices/{id}/binding`、`POST /v1/devices/{id}/binding/{unbind,supersede}`；自定义人格 `GET|POST /v1/personas`、`DELETE /v1/personas/{persona_id}`；监护人面 `GET /v1/guardian/minors/{minor_user_id}/summary`、`/v1/guardian/{links,notifications}`、`{delete,export}`。`pages/guardian` 已有邀请/绑定码/年龄段/授权开关（走 `/v1/guardian/*`）与 `pages/device` 的 app_confirm 切换，故剩余 UI 面窄于字面清单。线上若与仓库不一致，以线上为准并在此记录差异。
- 人格分配 UI 已接（`64a742f`，code；远端 CI `35306937489` 的 miniprogram job success：225 passed / 0 fail + 全量 `node --check`；本地同口径）：设备页新增按使用人分配/取消分配，列出 BindingManifest 的主要使用者/家庭成员并显示 override 优先、binding 默认兜底，抽屉从内置伙伴目录（starlight/taoxi/mianmian/axu/xuanmo）与账号自建人格（`GET /v1/personas`）选择后 PUT、或 DELETE 恢复默认；分配行显示名来自同一目录（未知 id 原样显示、不猜名字），提交只给 `persona_id`、版本由服务端决定；写入后回读服务端分配，失败不置成功态。自建人格接线 `6f22922`（同口径 miniprogram job success）；`9e0d2b4` 起自建人格可在新页 `pages/persona-custom` 创建/冻结/删除（结构化不可用时手填同一套受控字段），`b25268b` 补齐 custom_persona_id 派生用例（内置人格必须留空）。同时**退役 bio 标记死路径**：`utils/custom-persona.js` 与三处调用已删——服务端旧 bio 自由文本形态已移除（`interaction.py` 注明），客户端继续写 bio 是假能力；声音克隆改为绑定设备当前分配的自定义人格（仅 cu_* 且账号目录存在时提交）。**剩余（本轮已核现状）**：① 年龄资料在**绑定创建**流程已有（`pages/bind` 选年龄段 → `subject_draft.age_band`），建后单独申报 `PATCH /v1/persons/{id}/age-evidence` 未接；② guardian 页的邀请/授权动作已实现但被 `configActionGate("guardian_manage")` **无条件 fail-closed**（config seam 未接入），与声音撤销同一边界——需先定 consent 决策接口，不得绕过；③ 克隆未 ready 时回落设计音色的设备实听与运行中会话轮换（见上一条）。
...
- 依赖 P0-04/P1-03；先核已有真实音频校验、进度轮询、over_budget 逻辑是否在线，避免重复开发。**本轮已核查（scout 全量 + 抽查；结论=在线，不要重写）**：真实解码体检在 `services/voice_profile/sample_validation.py`（时长/采样率/RMS/有效语音/削波，不过门直接 422 且不落行），拒绝文案 `sample_copy.py`，进度与 60s 预算派生自持久化状态 `enrollment_progress.py`（queued/cloning/activating/done/failed，`PROMISED_TOTAL_MS=60_000`，`over_budget` 不阻断），生命周期与撤销删样本/删厂商音色 `manager.py`/`postgres_manager.py`（`status` 联合含 `failed`，`deletion_status` 独立），厂商客户端 `cosyvoice_enrollment.py`（120s 超时）/`doubao_voice_clone.py`，HTTP 面 `routes/voice.py`（consent POST/DELETE，enrollments 支持 `custom_persona_id`，ready-for-device 409 `reconciliation_required`，profiles/activate/DELETE profile），设备出声合同冻结 `companion_delivery.py::_attach_personal_clone`（provider/model/resource/过期 + 设计音色回落），Agent 侧 `voice_profile_client.py`/`agent_voice_profile.py`/`providers/doubao_tts.py`，小程序 `pages/profile`+`utils/api.js` 录音/预检/轮询/over_budget/422 文案，回归 `apps/miniprogram/tests/voice-enrollment-progress.test.js`。**四处缺口（① 已按代码证据修正）**：① ~~provider 失败终态缺失~~ **不是缺陷**：provider 结果不确定或失败时把 operation 置 `reconciliation_required` 且 profile 保持 `enrolling` 是**有意**的可恢复语义——`manager.reconcile_enrollment` + `pending_enrollments` 能把同一 enrollment 补完为 `candidate`（`services/voice_profile/tests/test_manager.py` 的 provider 持久化触发器用例：重放 409 → 对账后恢复成功），所以不得改成终态 `failed`。真正的缺口只是**用户可见信号**：超过 60s 预算后客户端只有"可以离开"，既没有"仍在处理"也没有"需要人工处理"的区分（`status='failed'` 实践中只由 `revoked` 触达）；② 客户端：**已接**（`9e0d2b4`）——我的页在 Runtime Profile 的 persona 为 cu_* 时随 enrollment 提交 `custom_persona_id`（仅账号目录里存在才提交，否则留空走账号本人声音）；旧服务端兼容分支同时剥离该字段。缺的是设备实听证据；③ 客户端撤销/样本处置被 `configActionGate` **无条件** fail-closed 挡住（`utils/device-binding.js::configActionGate` 对任何 action 都返回 `config_seam_pending`），privacy 页“撤回并删除”仍是弹窗占位——这是待决策的 consent 决策接口边界，不能绕过门去接 `DELETE /v1/voices/consent`；④ 缺真实证据：provider 克隆→设备可听端到端耗时（provider smoke 仍 `OFFLINE_MOCK`，且只覆盖设计音色）。
...
- 未见改写集已落地（`c48fcf9`）：`services/archive/evaluation/memory_eval_zh_v1_unseen.json`（跨会话计划/忌口转述/安慰式回忆/跨账号隔离 4 例），CLI `scripts/evaluate_memory.py --dataset …` 直接分报。规则路径基线 recall@5=0.6、nDCG@10=0.6、extraction_recall=1.0、source_attribution=1.0、leakage=0；两条未命中（『外卖该避开什么』的忌口转述、『我最近有点撑不住了』的安慰式提问）如实记录为当前天花板，**不在同一轮按它调参**；用例 `test_unseen_rewrite_set_reports_its_own_baseline` 钉住基线。
...
- **资源泄漏一项已定位并修复（`861bd87`，code；远端 CI `35313490964` success：Pytest 5114 passed/2 skipped、覆盖率 88%、orchestration 90%、provider protocols 95%，含模块预算门）**：CI 里 Agent interaction 用例退出时的 `interaction-delegation-start` pending 提示来自关闭路径自身——`close()` 先排空 `_background_tasks`，之后才 `await orchestrator.close()`，其间由回调新起的任务无人回收。现在 `close()` 为终结态且幂等，关闭后再调度会被拒绝（未启动协程显式 close，不静默丢包），并在 orchestrator 关闭后二次排空。实现放在新模块 `services/agent/src/runtime_shutdown.py`（`duplex_runtime.py` 有模块预算、该门只允许收紧不允许抬高）；回归用例触发真实委托后断言不残留任务、关闭后调度立即完成。**仍未做**：WSS 丢帧/乱序/重连与 Bridge/Agent 退出的故障注入、入队控制/error 帧在 lane 关闭后丢失的通用路径（Go 侧）、当前候选的 Go/Trivy 重扫。
...
- **首片已落地（`18d36ff`，code；远端 CI `35315844422` success：Pytest 5116 passed/2 skipped、覆盖率≥85%、orchestration 90%、provider protocols 95%）**：`services/companionship/evaluation.py` + 固定集 5 例 + CLI `scripts/evaluate_companionship.py`，每例都走产品自身 HTTP 面（真实绑定 → `/v1/sessions` → app_confirm 切人 → 权威签发签名 profile → 策略/监护同意），不注入 profile、不用桩。实测基线 5/5、accuracy 1.0、gate_violations 0、`unauthorized_recall` 0、p50≈284ms。覆盖：under_14/14_17 无同意 ⇒ 无会话能力且无私密记忆能力、成人自用 adult_companion、切人后旧签名 profile 被拒、监护记忆保留同意授予→撤回。**尚未覆盖**：模型措辞关怀度（离线不计分，需 recorded-bundle 人工盲评或设备窗口）、回顾可读性评分、模型超时负例的离线注入、未见集（当前场景是结构化门检查而非文本抽样，不做同义改写拆分）。
