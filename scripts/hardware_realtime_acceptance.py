...
This tool never flashes firmware, never changes production state, never uploads
anything, and never writes receipts: repository simulation cannot upgrade real
evidence.
"""
...
DEVICE_KEYS: frozenset[str] = frozenset(
    {"device_id", "board", "firmware_sha256", "runtime", "runtime_version"}
)
...
def _validate_scenario(item_id: str, scenario: object) -> list[str]:
    ...
    if item_id == "T1":
        ...
        _num(scenario, "cer", problems, min_value=0.0, max_value=1.0)
    elif item_id == "T5":
...
def _print_usage() -> None:
    print(
        """usage: hardware-realtime-acceptance {list,run,verify} [options]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"error: unknown command {command!r} (expected list|run|verify)", file=sys.stderr)
    return 2
