...
陪伴与关怀是产品内核，人群分三阶段扩展，任何对外叙事不得偏离此顺序：

1. **前期以学生为主**：陪伴、关怀、教学导师。未成年人链路走监护同意与学生账号能力门控（`services/control_api/app/account_gate.py`）。
2. **后期扩展老年人**：陪伴、关怀、人物复刻、人生故事搜集并制作成人生故事册。长期档案、声音克隆与证据链能力为该阶段底座。
3. **再往后扩展年轻人**：陪伴、潮玩。吉祥物 IP 在该阶段承接潮玩化。

机器人外形尚未确定；星澜、桃喜、绵绵、阿序、玄墨等吉祥物阵容为**占位资产**，商业计划书等对外材料可暂用，不绑定最终外形，后续可整体替换。

声纹门禁、按需档案、带标识导出与可证明删除是贯穿各阶段的能力底座，不是定位本身；「家庭桌面记忆终端 / 声纹档案音箱」的旧叙事不再作为定位表述，仅作能力描述保留。
...
- `RESEARCH.md`：外部研究扫描的唯一落地文件；开发在此评估并标记状态。这是文档预算的唯一例外，仍禁止 session 记录、ADR、组件 README、平行计划和 release note。
...
新账号先选择星澜、桃喜、绵绵、阿序或玄墨（阵容为占位资产，未绑定最终机器人外形，见「产品定位」）。主人声纹由小程序记录授权、机器人端采集，手机不获取录音；当前登记先生成 shadow 档案，只有 Speaker Authority 返回 `active` 后才能作为主人认证，也不得与声音克隆混为一谈。
