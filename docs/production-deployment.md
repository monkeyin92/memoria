...
叠层；失败时自动按切流前 Compose 配置恢复两个容器。薄发布只替换镜像，不修改 Agent 和
Voice Core Bridge 已有的 runtime authority tag。CI 同样按 Agent、Python、H5、Media Edge、
小程序变更范围选择门禁；Agent 薄发布必须等待 Agent job 成功，未修改组件的失败不得混入
