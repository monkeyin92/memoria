...
- `enabled`：后端已部署，开发版 **0.8.82** 已上传（瓷白控制面、无角色立绘，源 `d9911fd`）。正式审核/发布与体验版设置未做。源码与发布收据只记在 `HANDOFF.md`。
- `verified`：小程序单测 210/210、DevTools CLI 上传成功。0.8.82 尚未在手机/电脑微信或开发工具做真实验收；不得把 0.8.77 的 095c 开发工具结果说成本版三端通过。
