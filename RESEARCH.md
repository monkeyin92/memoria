...
- `enabled`：后端已部署，开发版 **0.8.77** 已从干净隔离源码上传。正式审核/发布、整套 UI 替换、生产绑定修改及权限放宽均未做。源码、镜像与发布收据只记在 `HANDOFF.md`。
- `verified`：最终发布源码 202/202、小程序编译 110 文件与前端 CI 通过；后端 CI 4,696 passed、9 skipped，另做真实生产 PostgreSQL/RLS 只读 canary，发现/详情一致、missing/foreign actor 被拒、绑定及角色不变。9 月 6 日 21:09 CST 开发工具新隔离项目从未登录恢复登录，首页/设备页可见设备，「我的」显示已绑定 1 台，尾号 095c、绑定 v2。不能把开发工具结果说成三端全部通过。
