...
本轮第三、四轮核对本地文档、源码、提交及 CI，并修改 P0-04/P0-03/P1-01 的代码与测试；第四轮新增 P0-04 的真实 persistent Session Runtime 链测试，并在本机既有 Docker PostgreSQL 容器上实跑（未构建镜像、未连接生产、未打开串口、未连接设备）。下列生产/板卡状态均为标注日期的既有收据，不是本轮实时健康证明；操作前须重新核验。
...
reviewed_source_commit: ca9dae16b59bc57a3ce0277b24867ffd8ed76066
...
code: committed_through_ca9dae1_plus_uncommitted_p0_04_runtime_chain_test
...
student_safety_local_scope: real_persistent_session_runtime_on_ephemeral_pg_with_sqlite_account_and_declared_guardian_outbox
...
2026-09-16 第二轮在**未提交的本地工作区**上改了 P0-04/P0-03/P1-01 的代码、测试与接线；没有新建候选、没有推送、没有 CI 结果。此后这些改动已提交（`e5f9d50`、`7c0ef48`、`ec56d9a`、`6ab16b9`），文档合并为 `28a41ee`/`ca9dae1`。第四轮新增一个未提交的本地测试文件（P0-04 运行时链），下述"已改"只表示 `code`（本地实现 + 定向测试通过），不是 `wired/enabled/verified`。
