...
当前 Agent/Bridge 发布提交为 `33617d357dfba03d593c0a808cb36f149fe04cf6`（2026-08-31 13:07 CST 切流，Agent-only source overlay，基座 `20260831-1305-clock-fact-pin-vad-guard-agent-component` / `3d3ed38`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1310-live-query-straddle-recover-agent-component`，image `sha256:93a69d7d292d78e2790e0551224bc77cb5f8db761cae9ab3ca0c541ed6dbb1ba`，revision `33617d357dfba03d593c0a808cb36f149fe04cf6`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「星期几成功后问南京天气，18 字完整 ASR 因 straddles_committed_without_timing 被拒，只提交 4 字碎片」：恢复跨已提交区间的 live/weather final 并在 media commit 时优先使用。紧邻回滚点 `memoria-agent:rollback-20260831-1310-live-query-straddle-recover-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1305-clock-fact-pin-vad-guard-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
