...
更新于 2026-09-17（同日晚继续：P0-04 剩余真实 PG 矩阵补齐 + 换管理账号 P0 读路径修复 + 本地全量 python CI 复跑，见下方带日期收据）。这里只保留当前运行基线、一个紧邻回滚、必要运维步骤和下一验收；完成过程与旧版本流水账已删除。唯一执行队列见 `TODOLIST.md`，后续完成项直接移出队列，不新增归档文档。
...
reviewed_source_commit: f67a134
...
code: committed_through_f67a134
