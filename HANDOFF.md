...
当前 Agent/Bridge 发布提交为 `4e36bc8d58f63fdf60d3bfb31eca833457b49aad`（2026-08-31 12:54 CST 切流，Agent-only source overlay，基座 `20260831-1200-clock-fact-overlap-recover-agent-component` / `8b47f66`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1255-clock-fact-projection-commit-align-agent-component`，image `sha256:b6a2489e066674f9a53157242cbe1acef026db925a7049493cd91158f22eaf59`，revision `4e36bc8d58f63fdf60d3bfb31eca833457b49aad`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「clock-fact 已识别且 turn_committed，但 projection_text_mismatch 吞掉播报、回没听清」：提交前用 canonical clock-fact 文本覆盖 timeline 拼接串，并在 `on_turn_committed` 后再次对齐 provisional。紧邻回滚点 `memoria-agent:rollback-20260831-1255-clock-fact-projection-commit-align-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1200-clock-fact-overlap-recover-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
