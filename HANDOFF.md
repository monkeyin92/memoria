...
- **已知缺口（不再靠加措辞）**：`mood-reason`（被老师批评）在最终版下 **6 次只存 3 次**（中间版 3/3 是靠过度记忆换来的）；`mood-happy-reason` 6/6。下一步建议做**确定性后处理**（原话含"明确情绪 + 具体原因"模式时由代码补一条 claim，不让模型自行判断），并用同一组正/负例验收；在此之前**不得宣称固定集稳定 1.0**。
