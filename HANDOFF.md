...
当前 Agent/Bridge overlay 源提交为 `96efeaacb249050b92c36efc34c35cf3b67cceab`（2026-09-03 14:31 CST 切流，标签 `20260903-1415-ledger-echo-guard-agent-component`）。Control API 与 Media Edge 发布提交仍为 `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`（2026-09-01 10:02 CST 切流，标签 `20260901-0945-wake-word-whitelist`）：

- Agent 与 Voice Core Media Bridge（容器 `memoria-agent-1` / `memoria-voice-core-media-bridge-1`）：`memoria-agent:20260903-1415-ledger-echo-guard-agent-component`，overlay revision `96efeaacb249050b92c36efc34c35cf3b67cceab`（含：① 未听过 `PREEMPTED` 允许 `FIRST_FRAME_SENT` 复活以便天气 live-lookup deep TTS 记 `actual_heard`；② media `on_media_playback_done` 戳 `_last_playback_completed_ns` 挡住 post-playback 星期回声幻影轮）。栈 `MEMORIA_RELEASE_TAG` 仍与 Control 对齐为 `20260901-0945-wake-word-whitelist` / commit `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`；Agent/Bridge healthy、bridge gRPC PASS、restart=0。自动切流因既有 override 含栈 tag 环境字段被 image-only 门禁拒绝，走手动 compose（`cutover_mode=manual_tag_split`）；上传构建收据 `/opt/memoria/component-releases/20260903-1415-ledger-echo-guard-agent-component/`。紧邻回滚点 `memoria-agent:rollback-20260903-1415-ledger-echo-guard-agent-component-pre-agent` 与 `-pre-bridge`（revision `6d13448354dfbca06bb69346fa24895bba1cb45c`，镜像 `20260903-1345-inflight-recovery-suppress-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。**待串口复测**（天气云端 `actual_heard` + 播完无「星期四」幻影轮）。**subject capability：blocked_ops**——演示账号仍 `subject_capability_forbidden`（2026-09-03 epoch 1365 仍 403）；需小程序微信手机号成人核验 → 声纹 enroll/activate，**不改门禁**。`direct_real_device_verified` 保持 false。
