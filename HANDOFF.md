...
- 当前工作区有一组**未提交、未部署**的 H5 语音止血与分层改动：新增
  `SpeechEpochAssembler`，以独立 ASR final segment 关联 LiveKit committed turn，不再假设
  “一个 VAD epoch 等于一个逻辑话轮”；新增严格白名单的服务端 `audio_trace` 校验和 H5
  `LiveKitAudioTelemetry`，记录实际麦克风设置、上行 RTP 与 FunASR 数值时序，不记录文本、
  设备 ID 或标签。H5 短打断 VAD 门槛从 `0.55s` 调为 `0.35s`，检测到候选插话时立即静音
  助手，误触发仍由现有语义/回声门禁恢复；首页恢复“停止回答”兜底按钮，但不结束会话。
  针对多 VAD 合并、迟到 final、旧 segment 隔离、回声前缀、相同连续话轮、短打断静音和
  遥测隐私均已补回归。390×844 本地浏览器已检查“停止回答”布局，无横向溢出或
  console warning/error；页面状态只用于视觉预览，真实按钮 handler 由组件测试覆盖，本轮没有
  请求麦克风权限。儿童真机识别率、短命令召回和外放听感仍需用同一组语料 A/B，不能把本地
  单测写成声学验收。
