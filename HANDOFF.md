...
当前 Agent/Bridge 发布提交为 `31bb37bfb468df0609a282352345d4ef886f1397`（2026-08-31 11:52 CST 切流，Agent-only source overlay，基座 `20260831-0955-half-duplex-asr-stall-rescue-agent-component` / `bc56f6b5dc53e3b59c618bfa36e0e6766a2f2599`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1150-clock-fact-endpoint-pin-agent-component`，image `sha256:37035bbace2e0c8e94e6dc7a3e14716c453b3b019c3505e3b3c59f5fbff943d3`，revision `31bb37bfb468df0609a282352345d4ef886f1397`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「识别到星期几却不提交、VAD 长尾拉长 endpoint 后走 LLM 乱答」：clock/date final 触发时钉住 endpoint 并零 grace 提交，后续 `vad.end` 不再扩展范围，提交时优先 timeline 内 clock-fact 文本。紧邻回滚点 `memoria-agent:rollback-20260831-1150-clock-fact-endpoint-pin-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-0955-half-duplex-asr-stall-rescue-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。2026-08-31 11:46 CST 操作员真机听感 pass：唤醒后说一遍「今天星期几」有完整回答（待补正式 receipt）。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
