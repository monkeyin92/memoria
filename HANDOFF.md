...
- 范围与红线：只改 `outputs/design/auto-audio-20260915/{auto_audio_session.py,auto_audio_analyze.py,selfcheck_analyzer.py}`（该目录仍受 `outputs/` 的 ignore 规则约束、**未入库**，本轮交付定位为本地可复跑诊断，不进 diff/commit；改动后 sha256 前 16 位：session `8e7b14f2d6ed4442`、analyze `888ccacfe86f9142`、selfcheck `e9517d72eaf156aa`）。未改系统音量/输入源/固件/生产配置，未开设备串口、未跑扫频、未切流。收据目录 `outputs/acceptance/run-20260921-tool-fixes/`。
...
- ② 参考匹配：**只放宽每块峰值的搜索窗**（`REFERENCE_CHUNK_SEARCH_S=0.60` 取代原先与晶格界共用的 ±0.15s），**`REFERENCE_MAX_SKEW_S` 维持 0.15 未动**（`selfcheck_analyzer.py` 有专门断言防止其被悄悄放宽）。新增**非门禁诊断**：`match_reference` 无论匹配与否都返回最强候选的 `chunk_ncc`/`chunk_residual_s`/`chunk_global_peak`/`spread_s`/`rejection`，`selftest_blocker` 把它带给操作者。证据 `reference-window-redgreen.py|.json`（几何取自 09-20 诊断的 0/−148ms/−170ms）：旧搜索窗下第三块**选中峰 NCC 0.036**（被窗裁掉），而它全库最强峰为 **0.429 @ −0.17s**，拒绝原因只有 `chunk_ncc_below_min` —— 旧实现分不清"峰被裁"与"波形缺失"；放宽搜索后三块选中峰均 ≥0.42、残差 0/−0.148/−0.170、`spread_s=0.17 > 0.15`，拒绝原因变为 `lattice_spread_exceeds_bound`。**该几何本轮仍被拒绝，且这是刻意的**：0.17s 的不一致源自一次未留下 WAV 的现场读数，究竟是取音失真还是峰值锁到邻近语音尚未确定，故不据此放宽晶格界；下一次真实取音的残差读数才是决策依据。
- ③ 3s/5s/8s 续问格（新增 `--follow-up-delays`（默认 `3,5,8`，最后一个值重复）+ `parse_follow_up_delays`/`planned_follow_up_delay`/`follow_up_anchor_local`/`wait_for_follow_up`/`seconds_between`/`follow_up_grid`）：锚点只取上一轮 `playback_ended_t`（缺失/不可解析则不发明延迟），等待受全局 deadline 约束并报 `cut_by_deadline`；每轮写 `follow_up.{planned_s,anchor_local,target_local,waited_s,started_local,actual_s,measure_point=afplay_spawn_not_audible_start}`（`actual_s` 由"afplay 启动时刻 − 锚点"测得，**不宣称可听间隔**）；`results["follow_up_grid"]` 记录 `covered/uncovered_delays_s` 并在未覆盖时打印提示。**覆盖语义已钉住**：默认 3 问只覆盖 3s、5s 两格，8s 需第 4 问（`--questions q1 q2 q3 q4 --follow-up-delays 3,5,8`），自检有一条断言专门证明"默认运行只覆盖两格并报出缺口"。非法值在播放前 exit 2（实测 `--follow-up-delays abc`）。
