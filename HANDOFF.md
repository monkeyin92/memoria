...
当前 Agent/Bridge overlay 源提交为 `e944376227c96b0dc35f588e3ee8e36c498fddf8`（2026-09-03 11:21 CST 切流，标签 `20260903-1055-cross-sentence-overlap-weather-recovery-agent-component`）。Control API 与 Media Edge 发布提交仍为 `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`（2026-09-01 10:02 CST 切流，标签 `20260901-0945-wake-word-whitelist`）：

- Agent 与 Voice Core Media Bridge（容器 `memoria-agent-1` / `memoria-voice-core-media-bridge-1`）：`memoria-agent:20260903-1055-cross-sentence-overlap-weather-recovery-agent-component`，overlay revision `e944376227c96b0dc35f588e3ee8e36c498fddf8`（含播放回声跨句 overlap 饿死天气轮修复）。栈 `MEMORIA_RELEASE_TAG` 仍与 Control 对齐为 `20260901-0945-wake-word-whitelist` / commit `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`；Agent/Bridge healthy、bridge gRPC PASS、restart=0。自动切流因既有 override 含栈 tag 环境字段被 image-only 门禁拒绝，走手动 compose（`cutover_mode=manual_tag_split`）；上传构建收据 `/opt/memoria/component-releases/20260903-1055-cross-sentence-overlap-weather-recovery-agent-component/`。紧邻回滚点 `memoria-agent:rollback-20260903-1055-cross-sentence-overlap-weather-recovery-agent-component-pre-agent` 与 `-pre-bridge`（revision `2290da1a80d974b91f78b59a8d4ac5c6cb67a976`，镜像 `20260901-1740-funasr-rescue-playback-asr-pause-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。天气轮修复已真机验收（`outputs/acceptance/half_duplex_investor_demo-20260903-1155.md`，session `3f9cb81f…` epoch 1360，天气+星期几两轮 actual_heard pass、owner_silence_timeout 正常关闭），但 receipt 缺串口 Device VAD start/end，且发现残留缺陷「TTS 回答回声可被提交为幻影轮」（turn4「星期四」），`direct_real_device_verified` 保持 false，不得仅凭本次 receipt 升级。**12:07 串口复测失败**（`outputs/acceptance/half_duplex_investor_demo-20260903-1237.md`，session `e332455a…` epoch 1361）：真人天气问句（rms=2389）被 cross_sentence_overlap 拒绝后恢复路径进入但提交静默饿死——`_recover_straddling_live_query_final` 仅在 can_add+ingest 成功时才重臂提交，turn_end_sample 未设置时 `_commit_pending_turn_after_grace` 静默 defer，60s VAD 抖动后 tail timeout 清空 forced text，设备无回答。e944376 只覆盖「拒绝后用户安静」场景，不覆盖「拒绝后 VAD 抖动」场景。**代码已补（待组件发布 + 串口复测）**：恢复路径无条件 `_observe_final_asr_result` + `_arm_live_query_forced_endpoint`（设 turn_start/end/endpoint 并重臂提交）；`live_query_forced_authoritative` 期间忽略后续 VAD start/end，grace 提交跳过 ASR coverage 等待；三处静默门禁（committed 水位 early-return / can_add|ingest 失败 / defer-with-forced-text）补 WARNING。回归：`test_device_weather_recovery_commits_despite_post_reject_vad_jitter` + 既有 overlap/straddle 单测转自动 grace 提交。板端 14 对 VAD start/end 齐全，固件无嫌疑；串口采集进程仍可复用。
...
stage_7: pass_2026_09_02_operator_receipt_pending_template_fields
