...
- 范围与红线：只改 `outputs/design/auto-audio-20260915/{auto_audio_session.py,auto_audio_analyze.py,selfcheck_analyzer.py}`（该目录仍受 `outputs/` 的 ignore 规则约束、**未入库**，本轮交付定位为本地可复跑诊断，不进 diff/commit；改动后 sha256 前 16 位：session `042adb9507a2fd75`、analyze `888ccacfe86f9142`、selfcheck `f744d66b8e2b6695`）。未改系统音量/输入源/固件/生产配置，未开设备串口、未跑扫频、未切流。收据目录 `outputs/acceptance/run-20260921-tool-fixes/`。
...
- ③ 3s/5s/8s 续问格（新增 `--follow-up-delays`（默认 `3,5,8`，最后一个值重复）+ `parse_follow_up_delays`/`planned_follow_up_delay`/`follow_up_anchor_local`/`wait_for_follow_up`/`seconds_between`/`follow_up_grid`）：**顺序修正**——可提问状态门（`ensure_listening`，必要时唤醒/重放欢迎语）现在先跑并把耗时记入 `wake_gates[].seconds_s`，延迟等待在其后对准**绝对目标时刻**（上一轮 `playback_ended_t` + delay），因此唤醒开销不会被算作续问延迟，门已越过目标时 `wait_for_follow_up` 返回 `already_past_target` 而不缩短等待；等待后只**检查**（不唤醒，避免移动被测延迟）设备是否仍在 `listening`，若已离开则以 `follow_up_lost_addressable_state` 停止并说明发生在哪个延迟上。每轮写 `follow_up.{planned_s,anchor_local,target_local,waited_s,cut_by_deadline,gate_seconds_s,addressable_state_after_wait,started_local,actual_s,late_by_s,measure_point=afplay_spawn_not_audible_start}`（`actual_s` 由"afplay 启动时刻 − 锚点"测得，**不宣称可听间隔**）。`results["follow_up_grid"]` 以 `tolerance_s=0.5` 判定：只有实际延迟不迟于计划值 +0.5s 的格才算覆盖，被唤醒吃掉或超时的格进 `late_delays_s`，未播的进 `uncovered_delays_s`，并在未覆盖时打印提示。**覆盖语义已钉住**：默认 3 问只覆盖 3s、5s 两格，8s 需第 4 问（`--questions q1 q2 q3 q4 --follow-up-delays 3,5,8`），自检有断言分别证明"默认只覆盖两格并报缺口""被唤醒吃掉的格计为 late 而非 covered"。非法值在播放前 exit 2（实测 `--follow-up-delays abc`）。
