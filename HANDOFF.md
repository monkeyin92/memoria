...
当前 Agent/Bridge 发布提交为 `3d3ed38ebf7785745afcd71367e6aa6399bf72bb`（2026-08-31 13:01 CST 切流，Agent-only source overlay，基座 `20260831-1255-clock-fact-projection-commit-align-agent-component` / `4e36bc8`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1305-clock-fact-pin-vad-guard-agent-component`，image `sha256:f79114e0b7b7f7fc7501fbfd2707c4a1386a33e786eecac3c6e1f72a59a7cfbf`，revision `3d3ed38ebf7785745afcd71367e6aa6399bf72bb`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「clock-fact 已识别并钉住 endpoint，但设备 VAD 重放 vad.start 清空待提交状态 + ASR coverage 等待，25s 后 tail timeout 回没听清」：钉住后忽略重放 vad.start、跳过 pinned endpoint 的 coverage 等待、禁止 stable-partial 与 pin 竞争。紧邻回滚点 `memoria-agent:rollback-20260831-1305-clock-fact-pin-vad-guard-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1255-clock-fact-projection-commit-align-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
