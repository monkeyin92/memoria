...
当前 Agent/Bridge 发布提交为 `3f81d47a8dccaf3ecf016bdc7c4931402e061210`（2026-08-31 15:35 CST 切流，Agent-only source overlay，基座 `20260831-1510-train-live-lookup-nudge-guard-agent-component` / `38f2ddd`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1535-live-lookup-semantic-router-agent-component`，image `sha256:3ed21aacb9e119aab2f3bf2fbcee0d169df1b8723c87efb6d2005da2ea254674`，revision `3f81d47a8dccaf3ecf016bdc7c4931402e061210`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片在关键词快路径之外增加小模型语义分类（`LIVE_LOOKUP_SEMANTIC_*`，默认 `deepseek-v4-flash`、0.8s 超时）：未命中 marker 的时效性问题先分类再决定是否走 Qwen forced_search；结果按归一化 query 缓存在 runtime。紧邻回滚点 `memoria-agent:rollback-20260831-1535-live-lookup-semantic-router-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1510-train-live-lookup-nudge-guard-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
