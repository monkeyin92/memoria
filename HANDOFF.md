...
- **待办**：① 用户本人听一次豆包声音；② 用户重新绑定设备以写入绑定时 consent grant，再验证 `memory_recall_private`（另需设备证书/attestation 有效）；③ 监护小结 consent 仍无写入方；④ media-edge 依赖易失的 `/tmp/media-runtime.override.yml`（已备份到 `.cutover/`；它只提供 `MEDIA_EDGE_DEVICE_WSS_ENABLED=true`，仓库 compose 已于 2026-09-26 固定该值，下次 media-edge 发布后可从 compose 链移除该文件；新镜像在生产未开设备 WSS 时拒绝启动）；⑤ `release-ops.sh finish` 的状态循环遇到无 healthcheck 容器会中断（本次已手工补跑），env 步骤没有断言 `MEMORIA_SPEAKER_AUTHORITY_ENABLED=false`，整栈发布脚本入库时两处都要修；⑥ 旧 20260827 树仍被数据层 bind mount，不可清理。
