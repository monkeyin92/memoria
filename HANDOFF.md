...
## 2026-09-21 DEMO-02：6 个固定场景接入记忆评测链路（离线规则收据 + 生产装配 4 次复跑，含容器内 1 次）
...
- 收据（`outputs/acceptance/run-20260921-demo02-recall/`，命令与 exit 见 `commands.txt`）：**离线规则提取器（CI 口径）** `extraction_recall=1.0`、`recall_at_5=5/6`、`cross_session_recall_at_5=1.0`、`comfort_recall_at_5=0.5`、`ndcg_at_10=0.688`、`extraction_precision=0.4`、`cross_account_leakage=0`（`report-rules.json`）。**生产装配三次复跑（本地当前代码 + 服务器密钥，transient、不打印不落盘）得到 `recall_at_5 = 0.5 / 0.5 / 0.333`**（`report-qwen.json`）。**容器内单次复跑（`report-qwen-incontainer.json`）**：`harness_file=/tmp/demo02-src/services/archive/memory_evaluation.py`（当前代码、16 场景，因镜像内只有 13 场景而临时覆盖；其余模块与密钥仍取 `/app` 与容器环境）、`key_chars=35`、`offline_mock=False`、`model=qwen-flash`、`extractor=FallbackMemoryExtractor`（非规则回落）、`case_count=6`、`failed_cases=()`、`adapter=memoria-sqlite-configured-in-container` → `recall_at_5=0.333`、`extraction_recall=0.5`、`comfort=0.0`、`tokens 3981 in / 1028 out`、`cross_account_leakage=0`。四次样本合起来：**0.5 / 0.5 / 0.333 / 0.333**。
- 生产容器事实（已核验，值未打印）：`ControlSettings.model_config.env_file=".env"` 而 `/app/.env` 不存在 ⇒ 设置来自 compose 注入的**环境变量**（`os.environ` 含 `DASHSCOPE_API_KEY`/`OFFLINE_MOCK` 键名）；`/etc/memoria-control-api.env` 在本容器不存在。跑本次评测的方式为：`tar` 当前 `services/archive/`（28 文件，排除 tests）→ 镜像内 `/tmp/demo02-src` 解包 → 用 `services.__path__` 前置补丁让归档模块取当前代码，`CatalogMemoryEvaluationAdapter` 仍用容器自带的容器内 temp SQLite（**未读未写生产 DB/配置**），跑完 `rm -rf /tmp/demo02-src`（已核验目录消失）。
- **DEMO-02 的三个实测发现（都在生产装配上，非规则路径）**：① 情绪场景那句"我今天被老师批评了，好难过。"**生产提取器一条都没存**（`qwen_probe.json`/`qwen_probe.py` 逐场景读数），因此该 case 在数据集里保留但**不带 review**（`_apply_reviews` 对无 claim 的 review 直接抛错会中断整轮），并在回归里以"可达性少这一例"钉住——修好它会立刻让测试失败；② 生产提取器存的**表面逐次漂移**（同一输入：`70-79` vs `70s`；`喜欢恐龙` vs `dinosaurs`；常为裸值而非句子），所以固定集的召回率也随之漂移（四次样本：0.5 / 0.5 / 0.333 / 0.333）⇒ **"固定集保成功率"这一前提在当前装配上不成立**，属于 P1-06/DEMO-02 的待办（换模型/改 prompt/加归一化）或需要产品显式决策，不能靠调数据集消掉；③ 数据集里的 `match_all` 因此跟着 **storyboard 措辞**（数学/七十几分、恐龙、纺织厂），不跟某次采样——把它钉到 `70-79` 这类瞬时值会是在 pin 噪声。
...
- 边界：数据集证明的是"固定集可复跑、可评分"，**不是产品召回质量、更不是部署栈行为**——四次生产装配运行中三次是**本地当前代码 + 生产密钥/模型**、一次是**容器内 + 当前 `services/archive` 临时覆盖**（镜像自带的 harness 是旧版，见上）；无论哪种都不是"部署后服务端到端"的召回证据。对外引用召回率前需 (a) 先处理上面的漂移与缺口，(b) 在部署了当前代码的栈上复跑。`extraction_precision`（规则 0.4 / 生产 0.154–0.3）刻意保留：它是提取器额外产出（episode/knowledge 等未评分项）的比例，不作为通过标准。
