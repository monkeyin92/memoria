...
更新于 2026-09-17（同日晚继续：P0-04 剩余真实 PG 矩阵补齐 + 本地全量 python CI 复跑，见下方带日期收据）。这里只保留当前运行基线、一个紧邻回滚、必要运维步骤和下一验收；完成过程与旧版本流水账已删除。唯一执行队列见 `TODOLIST.md`，后续完成项直接移出队列，不新增归档文档。
...
reviewed_source_commit: 9c04a35
...
code: committed_through_9c04a35
...
student_safety_local_scope: real_persistent_session_runtime_on_ephemeral_pg_with_sqlite_account_and_declared_guardian_outbox_real_pg_person_consent_gate_and_real_catalog_subject_key_isolation
...
`a8a0e43`（2026-09-16 16:12 CST）是上一轮已核对的本地与远端 `main`。CI `35072578098` 的 agent/python 通过；Edge、小程序、固件及镜像任务跳过，不能据此宣称这些制品通过。没有该候选上线的证据；最后记录的线上 Agent/Bridge 仍为 `d96d4c2`。 [2026-09-17 fix round reviewed 52241ed and a8ce4e1; CI 35184070037 failed at the Mypy step and the following jobs did not run; no candidate was built, pushed or deployed. 2026-09-17 continuation: local commit 30dea92 adds the remaining real-PG authority matrix tests and the full python CI was rerun locally and passed; a fresh GitHub CI run for 30dea92 is still pending, and nothing was deployed.]
...
- [fixed 2026-09-17, local code+tests+real PG on 5526c39, this continuation round] P0-04 remaining real-PG matrix + local full-CI rerun:
  ...
  - P0-04 profile-session mismatch cell + OPEN DEFECT on the legal-manager change (commit `9c04a35`, local code+tests+real PG, no production/device access): `test_both_policy_seams_reject_a_diverged_runtime_projection` forces the authority's projection row to diverge from the stored signed profile (the condition the nine-field guard exists for) and pins the split contract — `/session-policy` 503 `session_runtime_profile_binding_mismatch` (it must not decide), `/response-plan` a deliberately bounded conservative 200 (the crisis fixed reply must stay deliverable), with zero memory reads, no grounded items, no retention grant and no crisis evidence on either seam. Then `test_legal_manager_change_does_not_yet_close_the_agent_facing_seams` (characterization, explicitly labelled OPEN DEFECT) performs a real take-over on the authority plane — the ACTIVE binding goes `superseded` and another owner's binding version 2 becomes ACTIVE for the same device — and the seams keep answering for the OLD session: policy 200 with the account still the active subject, `conversation: true`, `owner_display_name` still projected, `/response-plan` 200 with one grounded item and a REAL account-keyed memory read (catalog delta 1), while `guardian.crisis_event` is correctly not recorded. The same authority's write path already refuses the identical state (`switch_subject` → `PersistentSessionDenied`, because `action_identity_lock_binding` validates version and status), and `PostgresSessionRuntimeService.current` only checks projection/epoch/TTL while `routes/interaction.py` never consults the binding or device trust — so this is a MISSING CHECK on the read path, not a design choice. Impact window = until the signed profile TTL expires (production default 5 minutes), during which the Agent is still told the session belongs to that subject and is still allowed the account-keyed memory read. NOT fixed here: the fix needs a new narrow `SECURITY DEFINER` predicate ("is this (device, binding_id, binding_version) still the device's ACTIVE binding") plus a grant to the Session role and a check inside `current()` next to `_require_current_profile` (the close-only path must still be able to close) — a schema/authorization change that needs explicit authorization first; the characterization test is its acceptance criterion (`account_keyed_reads_after_change` must become 0 and both seams must fail closed).
