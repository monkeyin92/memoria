...
当前 Agent/Bridge 发布提交为 `35300945757c0f3792122047501e93d0a720d8f4`（2026-08-31 19:04 CST 切流，Agent-only source overlay，基座 `20260831-1535-live-lookup-semantic-router-agent-component` / `3f81d47`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1905-compound-goodbye-standby-agent-component`，image `sha256:c99dc03d4cac9684fbe809032f701ef154903c02d0536a0628a8205b8dd13a21`，revision `35300945757c0f3792122047501e93d0a720d8f4`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「知道了，再见」未命中精确结束语、仍走 LLM 告别且播放后反复没听清」：复合 ack+告别（如「知道了再见」）视为 `conversation_end_explicit` 直接待命；若仍产生短告别回复，播放完成后按 `conversation_farewell_complete` 强制 standby 并跳过 missed-hearing nudge。紧邻回滚点 `memoria-agent:rollback-20260831-1905-compound-goodbye-standby-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1535-live-lookup-semantic-router-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
