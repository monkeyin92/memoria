...
- 当前：`memoria-agent:20260914-straddle-rescue-fix-v1`，image `sha256:f47dc87efcdd3a7841c121a10399a0aaa267575b6fc56a280a1041d29e969c16`，OCI revision `11643528b1244a7dec57afcb4d9a9b1eb818bc16`。Agent/Bridge 共用该 image，2026-09-14 **17:48:05 CST**（09:48:05Z）切流；`component_manifest=PASS`、`bridge_grpc_socket=PASS`、双 `healthy`；容器内 `/app/services/agent/src/voice_core/media_session_commit.py` 已含本次修复代码（两容器 `grep -c "final is mostly committed"` 均为 1），readiness `ready`、agent `ready`、栈 tag 仍为 `20260901-0945-wake-word-whitelist`。收据 `/opt/memoria/component-releases/20260914-straddle-rescue-fix-v1/`。
- 紧邻回滚：`memoria-agent:rollback-20260914-straddle-rescue-fix-v1-pre`（内容即上一版 `memoria-agent:20260913-p0-empty-input-resume-v1`，image `sha256:de42440fad5fddd7afcb5bc67472bd7eaf084a91b5f0f9fa8970bbee7d768072`，revision `6ca25f71da05ec1cf0c08d46ce4b92c643a8f466`）。切前后回滚 tag 与两个服务的 override 及镜像 ID 均已核实。**回滚只证明可运行，旧版仍会在「一次提问被提交成两个轮次」时丢掉第一遍答案。**
