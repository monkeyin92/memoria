...
- Agent 与 Voice Core Media Bridge（容器 `memoria-agent-1` / `memoria-voice-core-media-bridge-1`）：`memoria-agent:20260903-1248-live-query-vad-jitter-recovery-agent-component`，overlay revision `1b49eed6ea7607344b4a8871a0914b6f85c6d128`（含拒绝后 VAD 抖动饿死 live-query 恢复修复）。栈 `MEMORIA_RELEASE_TAG` 仍与 Control 对齐为 `20260901-0945-wake-word-whitelist` / commit `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`；Agent/Bridge healthy、bridge gRPC PASS、restart=0。自动切流因既有 override 含栈 tag 环境字段被 image-only 门禁拒绝，走手动 compose（`cutover_mode=manual_tag_split`）；上传构建收据 `/opt/memoria/component-releases/20260903-1248-live-query-vad-jitter-recovery-agent-component/`。紧邻回滚点 `memoria-agent:rollback-20260903-1248-live-query-vad-jitter-recovery-agent-component-pre-agent` 与 `-pre-bridge`（revision `e944376227c96b0dc35f588e3ee8e36c498fddf8`，镜像 `20260903-1055-cross-sentence-overlap-weather-recovery-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。天气轮修复曾真机验收（`outputs/acceptance/half_duplex_investor_demo-20260903-1155.md`，session `3f9cb81f…` epoch 1360），但 receipt 缺串口 Device VAD start/end，且发现残留缺陷「TTS 回答回声可被提交为幻影轮」（turn4「星期四」）。**12:07 串口复测失败**（`outputs/acceptance/half_duplex_investor_demo-20260903-1237.md`，session `e332455a…` epoch 1361）：真人天气问句被 cross_sentence_overlap 拒绝后，恢复路径未无条件重臂提交，VAD 抖动静默 defer 至 tail timeout。**本切片已修并切流**（恢复无条件设 turn bounds + authoritative 期间忽略后续 VAD + 三处 WARNING；回归 `test_device_weather_recovery_commits_despite_post_reject_vad_jitter`）。`direct_real_device_verified` 保持 false，待串口复测（采集进程仍可复用）。板端固件无嫌疑。
