...
当前 Agent/Bridge 镜像为 `memoria-agent:20260906-0048-companion-custom-voice-agent-component`（源 `4edfaa576dd8a11c3933579f1276ec6f18ca60cf`）。Control API overlay 源提交为 `4edfaa576dd8a11c3933579f1276ec6f18ca60cf`（标签 `20260906-0048-companion-custom-voice-control-api`）。Agent/Bridge/Control 的 env `MEMORIA_RELEASE_TAG` 均为 `20260901-0945-wake-word-whitelist`。**2026-09-04 文本模型切流（env）**：`LLM_PROVIDER=qwen`，主对话 `QWEN_FAST_MODEL=qwen3.7-flash`（关思考），分类器 `qwen-flash`（打断/联网/告别/危机/摘要/记忆抽取）。联网查询隔离源仍用 `QWEN_DEEP_MODEL=qwen-plus`。不再用即将下线的 `deepseek-v4-flash` 当对话模型，也不迁到更贵的 `deepseek-v4-flash-0731`。

- Agent 与 Voice Core Media Bridge（容器 `memoria-agent-1` / `memoria-voice-core-media-bridge-1`）：`memoria-agent:20260906-0048-companion-custom-voice-agent-component`，revision `4edfaa576dd8a11c3933579f1276ec6f18ca60cf`，image `sha256:f6c8f49fbbff42eb95f2358bf67276eead466da11c14ffa520185e0a26c7d759`。两者 **healthy**、restart=0。收据 `/opt/memoria/component-releases/20260906-0048-companion-custom-voice-agent-component/`。回滚 `rollback-20260906-0048-companion-custom-voice-agent-component-pre-agent/-pre-bridge`（镜像 `20260905-lookup-voice-bind-agent-component` / `sha256:55f8f6d416725b681098a0ad5e2e2571d6506ca85a1fbecde1d733a0451fd620`）。**subject：「主人」已 adult/verified；声纹 profile `1b5b577b` 已 active。** `direct_real_device_verified` 保持 false。天气正文与车票垫话待真机复测。自定义人格/目录人格换声待下次唤醒真机复测。
...
`wired=production_control_and_agent`；`enabled=true`：2026-09-06 00:44 CST Control overlay 切流，00:46 CST Agent/Bridge 切流。两者 revision `4edfaa576dd8a11c3933579f1276ec6f18ca60cf`，healthy、restart=0；栈 env 仍为 `20260901-0945-wake-word-whitelist`。其余 13 个运行容器未变。

`verified=2026-09-06_00:46_CST`：源提交门禁（ruff / module-budget / mypy / Agent unit）、Control 8 文件前后哈希与 import smoke、Agent overlay import smoke、切流后 live import 与 health。**未做真机唤醒复测**；换人格或自定义声音要下次唤醒才生效。自定义克隆必须已经是 active；未就绪时仍用上次目录设计音色。小程序文案已进 git，微信开发版未随本轮上传。

发布/回滚收据：Control `/opt/memoria/component-releases/20260906-0048-companion-custom-voice-control-api/`；Agent `/opt/memoria/component-releases/20260906-0048-companion-custom-voice-agent-component/`。
