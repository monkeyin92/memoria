...
## 2026-09-21 模拟音频工具：两处自检判据修复 + 3s/5s/8s 续问格（未跑设备）

- 范围与红线：只改 `outputs/design/auto-audio-20260915/{auto_audio_session.py,auto_audio_analyze.py,selfcheck_analyzer.py}`（该目录仍受 `outputs/` 的 ignore 规则约束、**未入库**；改动后 sha256 前 16 位：session `e946496f1ae89f7a`、analyze `fa1700d8eb8ffedc`、selfcheck `d879a4cc9371d6d2`）。未改系统音量/输入源/固件/生产配置，未开设备串口、未跑扫频、未切流。收据目录 `outputs/acceptance/run-20260921-tool-fixes/`。
- ① live 门禁（`--ffmpeg-start-timeout-s` 默认 10→`FFMPEG_START_TIMEOUT_S=25.0`；转发 ffmpeg `-progress pipe:2`，新增 `Recorder.progress/bytes_written/file_bytes`，blocker 带 `file=…B progress=…`）：原因是判据只看 WAV 文件，而 ffmpeg 8 的 ~256 KiB muxer 缓冲要 ~10s 才落盘。证据：`flush-probe.json`（同 argv、`-t 12`、wall 12.38s，首字节 **10.09s**，文件时长 10.111s，日志空）与 2026-09-20 现场"约 10s 才刷到 256KB"一致；`progress-probe.json` 证明 `-progress` 在 **0.84s** 报 `total_size=13420`，即正常路径 ~1s 内判定、上限只作无 progress 构建的回退。
- ② 参考匹配（`REFERENCE_CHUNK_SEARCH_S=0.60` 取代 per-chunk ±0.15s 搜索窗；`REFERENCE_MAX_SKEW_S` 0.15→0.30；匹配记录新增 `chunk_residual_s`/`chunk_global_peak`/`chunk_search_s`；NCC 阈值 0.20 与唯一候选要求不变）：`reference-window-redgreen.py` 复现 2026-09-20 诊断的几何（三块参考落在晶格 0/−148ms/−170ms），三个变体显示旧界与"仅放宽搜索"都仍是 `reference_not_detected`，**两界同时放宽**才 `reference_located`（chunk NCC 0.425/0.419/0.429，残差 0/−0.148/−0.170，`start_bounds_s=[3.83,4.0]`、`end_s=6.7` 覆盖真实播放）。语义未放宽：存在性仍由每块 NCC + 唯一候选决定，放宽的只是"位置可被拖多远"。
- ③ 3s/5s/8s 续问格（新增 `--follow-up-delays`（默认 `3,5,8`，最后一个值重复）、`parse_follow_up_delays`/`planned_follow_up_delay`/`follow_up_anchor_local`/`wait_for_follow_up`/`seconds_between`）：锚点只取上一轮 `playback_ended_t`（缺失/不可解析则不发明延迟），等待受全局 deadline 约束并把 `cut_by_deadline` 报出；每轮写 `follow_up.{planned_s,anchor_local,target_local,waited_s,started_local,actual_s,measure_point=afplay_spawn_not_audible_start}`，`actual_s` 由"afplay 启动时刻 − 锚点"测得，**不宣称可听间隔**。非法值在播放前 exit 2（实测 `--follow-up-delays abc`）。
- ④ 设备签名 barge 源（容器内只读读 `DeviceSettingsAuthority.current().allowed_barge_in` + `settings_version`，经 `barge_source_evidence` 写入 `results["bindings"]`）：读不到即 `allowed_barge_in=None, source=unknown`（不假设允许），错误另存 `barge_settings_error`。用于下一设备窗口分判 q3/q5/q8 与被禁源 barge；未在真实容器内执行过该读取（本轮只测解析与记录语义）。
- 验证：三套自检全绿 exit 0（`selfcheck-analyzer.txt` 含新增 7 项参考窗、6 项 live 门禁、15 项延迟、4 项 barge 源；`selfcheck-session-evidence.txt`、`selfcheck-volume-restore.txt`）；`ruff check` 六个工具文件 clean；三文件 `py_compile` OK；`--dry-run` 在无 firmware receipt 且本机静音时正确拒绝。
- **未验（不得宣称）**：扫频本身未跑——需机器人、解除输出静音、有效 firmware receipt 与空闲串口；F1 维持"部分观察"、F2 维持"未验证"；3s/5s/8s 三格需设备窗口实测；真实几何的残差也要等下一次真实取音（工具现已保留录音并输出每块 NCC/残差/全局峰）。
- 旁证（本地进程上下文，需 Terminal 侧对照）：同 argv 取音的 WAV 比墙钟短 —— `-t 20` → ffmpeg 自身 `out_time 20.0s` 但 WAV 仅 16.633s（差 3.367s）；44.1kHz 原生与 16kHz 同缺（15s 取音 2.554s vs 2.473s）⇒ 与重采样无关，**疑似本进程音频上下文丢样**，而 2026-09-20 的 Terminal 运行是 20.40s/20s 满长。这可能是既有 `inconsistent_sample_wall_clock` 的另一种解释，但**证据不足**，须在 Terminal 复跑同一探针后才能定论。
