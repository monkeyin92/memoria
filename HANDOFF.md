...
当前 Agent/Bridge overlay 源提交为 `6d13448354dfbca06bb69346fa24895bba1cb45c`（2026-09-03 13:51 CST 切流，标签 `20260903-1345-inflight-recovery-suppress-agent-component`）。Control API 与 Media Edge 发布提交仍为 `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`（2026-09-01 10:02 CST 切流，标签 `20260901-0945-wake-word-whitelist`）：

- Agent 与 Voice Core Media Bridge（容器 `memoria-agent-1` / `memoria-voice-core-media-bridge-1`）：`memoria-agent:20260903-1345-inflight-recovery-suppress-agent-component`，overlay revision `6d13448354dfbca06bb69346fa24895bba1cb45c`（含回复进行中禁止重复 clock-fact/live-query 恢复提交，避免取消首发 TTS）。栈 `MEMORIA_RELEASE_TAG` 仍与 Control 对齐为 `20260901-0945-wake-word-whitelist` / commit `7ca3d4ec531305d968d67ef1bb13b944e566e4cf`；Agent/Bridge healthy、bridge gRPC PASS、restart=0。自动切流因既有 override 含栈 tag 环境字段被 image-only 门禁拒绝，走手动 compose（`cutover_mode=manual_tag_split`）；上传构建收据 `/opt/memoria/component-releases/20260903-1345-inflight-recovery-suppress-agent-component/`。紧邻回滚点 `memoria-agent:rollback-20260903-1345-inflight-recovery-suppress-agent-component-pre-agent` 与 `-pre-bridge`（revision `1b49eed6ea7607344b4a8871a0914b6f85c6d128`，镜像 `20260903-1248-live-query-vad-jitter-recovery-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。**12:54 phase-2 PASS**（epoch 1362）。**13:03 阶段 7**（epoch 1363）：星期几 gen3 曾 `reply_task_exception`。**13:54 串口复测 PASS**（`half_duplex_investor_demo-20260903-1357.md`，session `e905fe17…` epoch 1365）：天气 live-query forced；星期几 **首发 gen3** `actual_heard=True`/`playback_ended=True`，无 turn4/gen4、无 `reply_task_exception`；listening→idle。天气云端标准 `actual_heard` ledger 仍为下一独立项。`direct_real_device_verified` 保持 false。
