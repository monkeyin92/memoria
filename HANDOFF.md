...
当前 Agent/Bridge 发布提交为 `38f2dddc6da8ed861e12e37edb50b90271df93b0`（2026-08-31 15:12 CST 切流，Agent-only source overlay，基座 `20260831-1310-live-query-straddle-recover-agent-component` / `33617d3`），Media Edge 发布提交为 `4c3971fef0bfdfc30e9bff742c40ffdd848c0e7c`（镜像不变），Control API 当前部署提交为 `349a3a3c5a40e350b82fb16f23e5f33e482a3f81`：

- Agent 与 Voice Core Media Bridge：`memoria-agent:20260831-1510-train-live-lookup-nudge-guard-agent-component`，image `sha256:bccd9b0c87bab0616851dd38c021f3f6c310474de8c2710dc9eb995b2a88f9be`，revision `38f2dddc6da8ed861e12e37edb50b90271df93b0`。两个容器 healthy、bridge gRPC PASS、restart=0。本切片修复半双工真机「动车/列车查询未走实时搜索、LLM 答没法查询」与「再见 ASR 碎片后空转写反复没听清」：agent 侧 `requires_live_media_lookup` 补「动车」「列车」标记；missed-hearing nudge 加 12s 冷却、最多 2 次后强制 standby。紧邻回滚点 `memoria-agent:rollback-20260831-1510-train-live-lookup-nudge-guard-agent-component-pre-agent` 与 `-pre-bridge`（回滚镜像为切流前 `20260831-1310-live-query-straddle-recover-agent-component`）。设备会话仍 `barge_in_enabled=false`；DTLN `8.0x`、PCM tap 仍在 bridge `/tmp/media-pcm-tap`。不得仅凭听感把全局 `direct_real_device_verified` 改为 true。
