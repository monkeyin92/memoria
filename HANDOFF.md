...
- 设备窗口（2026-09-20 17:33–17:48，已启用候选 `memoria-agent:20260916-livekit-181-v1`（rev `d96d4c2`）；**不是**冻结候选 `b668960`）：`scripts/voice_session_capture.py --out outputs/acceptance/run-20260920-p0-03-device-live-enabled-d96d4c2 --firmware-receipt outputs/acceptance/run-20260915-p0-03-metering-lifecycle-device/postflash.json --server-logs --duration 900` → `CAPTURE_STOPPED 17:48:34`、`capture_integrity=completed`、`log_stream_health=healthy`、`cleanup_errors=0`、`exit_reason=duration_elapsed`；固件收据为 2026-09-15 18:51 那次刷机（`release_head=d1ad38fe…`、`candidate_app_sha256=7d95c8…`，是操作员刷机收据而非板上实时读取），此后无重刷记录，属**假设**。`audible=not_measured`（需设备播放终态 + 操作员听感）；`agent.log` 为空（该容器无输出）属观测缺口。
  ...
  - **发现 F1（产品判定：体验不可接受，用户 2026-09-20 明确）**：播后约 4.4s 就 `owner_silence_timeout` 待命（`edge.log 09:35:22.645 reason=owner_silence_timeout` → 串口 `listening -> idle`），第三轮续问因此无法自然接上。机制：`MEDIA_OWNER_SILENCE_TIMEOUT_S` 生产未设置（默认 10s），预算只在 `listening` 相位走表、`speaking` 暂停（`services/agent/src/voice_core/media_session_standby.py:236-256`），播后只**沿用剩余量**而不是重新给足，所以多轮对话的续问窗口越来越短。待决：语义修复（播后 `reset=True` 重新给足整段）与/或把 env 提到 30–60s；两者都需设备窗口复验。不得用"延长静默"掩盖轮次边界问题本身。
  - **发现 F2（缺陷）**：待命后立即再唤醒被拒——串口 `MemoriaProtocol: Server closed media session: code=barge_source_forbidden retryable=0`（17:41:45.122）→ `listening -> idle` → 屏幕 `Alert [cancel] 错误: 设备媒体会话被服务端终止` → `Device WebSocket disconnected attempt=3`；`edge.log 09:41:45.044 device WSS handler rejected … kind=text`。根因：`services/media_edge/device_ws_uplink.go:133/171/217` 中 `handleVAD` 的 `vad.start` 在 `isPlaybackActive()` 时被判为 voice barge，而设备设置 `AllowedBargeIn` 不含 `voice`（P1-07 边界：当前只放行 button/keyword，voice 需签名授权）。策略本身符合 P1-07，缺陷在**代价是整段会话终止 + 用户可见错误**。待决：固件侧播放期不发 `vad.start` / Edge 侧降级为忽略该帧不终止会话（保留 fail-closed），voice barge 长期走 P1-07 签名授权。
