...
- 本地 `main` 上曾有仅用于生产取证的 `[DEBUG-aec-gate-20260726]` 日志提交；该临时提交
  已从未推送历史中移除，相关两个源码文件内容与 `a0308a4` 一致。
- 本地提交未推送。不要把临时 DEBUG 取证重新带入候选。

## 当前生产

- runtime：`20260726-181813`，于 2026-07-26 20:15:20 CST 原子切换。
- H5：`20260723-192611`。
- 小程序体验版：`0.8.49`。
...
- 当前版本：`20260726-181813`，待真机验收。
- 直接回滚：`20260726-133033`。
...
- 旧红新绿：
  ...
  - 候选全量 Python：`1234 passed, 27 skipped`。
...
- 当前只保留主工作树；已合并/诊断临时 worktree 和本地分支已删除。
