...
## 2026-09-21 确定性情绪补录（mood follow-up）：mood-reason 不再交给模型判断（本地 code，未提交/未部署）
...
- **边界（勿越界）**：① 本轮未调用生产密钥，configured 固定集未重跑——"固定集稳定 1.0"的表述在重跑前**仍不恢复**；② 未部署（容器 overlay/镜像仍是旧版），提取层保证只对下次发布的装配生效；③ **minor 主体边界**：生产会话链对 minor 的长期投影有既有 P0-04 收窄（`filter_extraction_for_subject`：daily_life 需 `low_risk_daily`），`predicate="mood"` 的 claim 在 minor 下仍会被拦——本轮只保证**提取层**存在，不改 minor 策略；评测 harness（无 subject category resolver）不受影响；④ 未提交（等指示）、未触碰设备/串口/生产。
